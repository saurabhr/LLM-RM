...
#SBATCH --output=out_rm_rrrr_%j.log
#SBATCH --error=err_rm_rrrr_%j.log
...
taskname_file=./simulation/taskrmllm_episodic_chain_all.txt
modelname_file=./simulation/llmmodels_test_remote.txt
run_script=./simulation/run_rm_task_episodic_no_fb.py # change it to run single turn and trial chain script.

# To run the single-turn chain instead of the episodic conversation chain,
# comment out the taskname_file and run_script lines above (leave modelname_file
# as-is — it's shared by both variants) and uncomment these two:
# taskname_file=./simulation/taskrmllm_single_turn_train_chain_all.txt
# run_script=./simulation/run_rm_task_single_turn_trial_chain.py
